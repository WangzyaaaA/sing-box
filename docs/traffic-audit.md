...
> 流量审计会记录来源 IP、目标地址、端口、用户标识、协议、路由和流量等信息。
...
总流量按核心累计计数增量持久化；核心重启导致计数回退时，服务会自动建立新基线，
不会清空既有历史数据。连接明细每秒采样，持续时间短于采样周期的极短连接可能不出现在
明细表中，但其字节数仍会进入总流量统计。
...
```json
{"ok":true,"service":"sing-box-audit","version":"1.0.0","auth_required":true}
```
...
- 总流量、上传、下载和活动连接数。
- 当前上传、下载和总速率。
- 1 小时、6 小时、24 小时、7 天、30 天或全部历史的流量趋势。
- 热门目标地址。
- 连接开始时间、来源、目标、用户/入站、协议、路由、上传、下载和状态。
- 按关键词、协议和连接状态筛选。
- 分页浏览。
- 按当前时间范围和筛选条件导出 CSV 或 JSON。
...
来源和目标、用户、协议、进程、出站链、规则以及上传/下载字节数。时间字段使用 Unix
时间戳；Web 页面会按浏览器本地时区显示。
...
上面的命令表示删除 30 天以前的流量采样和已结束连接。活动连接不会被删除。
...
- 连接明细来自每秒活动连接采样，极短连接可能在两次采样之间完成。
- 某个连接跨越所选时间范围时，其累计字节和流量时间桶的归属方式不同。

做容量统计时以页面总流量和趋势为准；做连接调查时使用明细与导出记录。
